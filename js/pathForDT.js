 ...
 * DTGeometry
 */
class DTGeometry extends THREE.BufferGeometry {
  /**
  ...
  /**
   * Update geometry by DTPointList instance
   * @param {DTPointList} pathPointList
...
/**
 * DTPointList
 * input points to generate a DTPoint list
 */
class DTPointList {
  constructor() {
  ...
  set(points, cornerRadius = 0.1, cornerSplit = 10, up = null, close = false) {
    ...
    if (points.length < 2) {
      console.warn('DTPointList: points length less than 2.');
      this.count = 0;
      return;
    }
 ...
 * DTTubeGeometry
 */
class DTTubeGeometry extends DTGeometry {
  /**
   * @param {Object|Number} initData - If initData is number, geometry init by empty data and set it as the max vertex. If initData is Object, it contains pathPointList and options.
   * @param {Boolean} [generateUv2=false]
   */
...
export { DTPoint, DTGeometry, DTPointList, DTTubeGeometry };
